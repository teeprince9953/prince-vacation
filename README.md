...
이번 버전에는 공휴일 반영 기능이 포함되어 있습니다.

- 달력에서 공휴일은 빨간색 라벨로 표시됩니다.
- 관리자는 `관리자 설정 → 공휴일 관리`에서 공휴일을 직접 추가하거나 수정할 수 있습니다.
- 입력 형식은 한 줄에 하나씩 아래처럼 입력합니다.
...
- 팀원이 선택한 휴가 주차 안에 공휴일이 포함되어 있으면 선택 완료 후 아래 안내 팝업이 표시됩니다.

```txt
선택 기간에 공휴일이 1일 포함되어 있습니다.
연차 +1을 대표님에게 보고해주세요 📝
```

- 공휴일이 2일 이상 포함되면 포함된 일수만큼 `연차 +2`, `연차 +3`처럼 자동 계산됩니다.
- `해당 연도 기본 공휴일 적용` 버튼은 현재 2026년 기본값을 제공합니다. 다른 연도는 관리자 화면에서 직접 입력하면 됩니다.


## 최근 수정: 공휴일명 가독성 개선

- 달력의 공휴일 배지 글자가 보이지 않던 문제를 수정했습니다.
- 공휴일명은 빨간 배지 안에 흰색 글자로 표시됩니다.
- 공휴일명이 비어 있는 기존 저장 데이터도 `공휴일`로 보이도록 처리했습니다.

## 이번 수정 사항

- 공휴일명 표시 방식을 빨간 배지에서 빨간 글자만 표시하는 방식으로 변경했습니다.
- 공휴일 날짜 숫자와 날짜 칸의 연한 붉은 강조는 유지됩니다.
- 공휴일명은 배경, 테두리, 그림자 없이 빨간 텍스트로만 출력됩니다.
